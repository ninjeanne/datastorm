...
def etl(inputs, outputs):
    ...
    ghcn_data = 'data/ghcn/'
    ghcn_df = spark.read.format("s3selectCSV").schema(observations_schema()).options(compression='gzip').load(
        inputs + ghcn_data)
...
if __name__ == '__main__':
    inputs = sys.argv[1]
    outputs = sys.argv[2]
    spark = SparkSession.builder.appName('Datastorm ' + str(NUM_OF_PARTITIONS) + 'p').getOrCreate()
